...
--- &text
First, we will define a term - Expression. An expression can be thought of as a rule for searching for a match. Secondly, a Target String is defined as a string within which a desired match will be looked for. To begin with, we will look at the simplest expressions, those containing only Literals. A literal is a character or a string template against which an exact match will be searched for.
...
Use the anchors and `{}`
*** .ans_tests
regexTest(e$val, "^A[A-Z]{3,5}E$", list("AZURE", "ADLKJE", "ADDDDDZ", "AZZE", "AE", "SDSDAEEEE", "DSDAlkdfE"))
...
--- &cmd_question
The last metacharacter we will look at is the pipe `|`, which is the syntax for or. For example "gr(a|e)y" will match "grey" or "gray". Now it is your turn, come up with the expression that will match a string that starts with "Lord", "Load", or "Lodd" using only "()" and "|" and save that expression to Z.
```{r}
"^(Lo((r|a)|d)d)"
```
*** .hint
